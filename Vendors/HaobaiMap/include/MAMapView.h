...
//#import "MAConfig.h"
#define MAMAPVIEW_CONFIGFILENAME  @"map_config.xml"
...
//开启通过双指转动，来旋转地图
@property(nonatomic, assign) BOOL twofingerRotate;
//开启通过设备转动，来旋转地图
@property(nonatomic, assign) BOOL deviceRotate;
/// 设定是否显示定位图层
...
//当前用户的定位圆
@property(nonatomic,retain) MACircle* accuracyCircle;
/**
 *@param frame 初始化地图的大小
 *@param mapType 初始化地图的类型
 */
-(id)initWithFrame:(CGRect)frame withMapType:(MAMapType)initMapType;

/* 设置地图旋转角度
 *@param地图旋转的角度。以度为单位
 */
-(void)setMapAngle:(int)angle;
/**
...
/*复用annotationview  
 *@param identifier annotationview的唯一标识
 */
-(UIView<NIRecyclableView>*)dequeueResuableAnnotationWithIndentifier:(NSString*)identifier;
@end
...
/**
 *根据传入的旋转方向，返回YES or NO 来自适应比例尺跟图标的位置。
 */
-(BOOL)mapView:(MAMapView*)mapView shouldAutorotateToInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation;
@required
/** 
 **发现离线地图包
 **返回YES，继续解压缩。返回NO，跳过解压缩。
 **/
-(BOOL)mapView:(MAMapView *)mapView zipFindorWillGo:(NSString*)info;
 ...
 **通知用户开始解压缩离线地图数据包
 **/
-(void)mapView:(MAMapView *)mapView zipWillDecompression:(NSString*)info;

/**
 **结束解压离线地图数据包
 **通知用户解压缩离线数据包完毕
 **/
-(void)mapView:(MAMapView *)mapView zipEndDecompression:(NSString*)info;
