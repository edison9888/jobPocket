    ...
    * BASE64_UNIT_SIZE;
    ...
    while (true)
    {
        ...
        for (; i + BINARY_UNIT_SIZE - 1 < lineEnd; i += BINARY_UNIT_SIZE)
        {
            ...
            //
            outputBuffer[j++] = basis_64[(inputBuffer[i] & 0xFC) >> 2];
            outputBuffer[j++] = basis_64[((inputBuffer[i] & 0x03) << 4)
                                                   | ((inputBuffer[i + 1] & 0xF0) >> 4)];
            outputBuffer[j++] = basis_64[((inputBuffer[i + 1] & 0x0F) << 2)
                                                   | ((inputBuffer[i + 2] & 0xC0) >> 6)];
            outputBuffer[j++] = basis_64[inputBuffer[i + 2] & 0x3F];
        }
