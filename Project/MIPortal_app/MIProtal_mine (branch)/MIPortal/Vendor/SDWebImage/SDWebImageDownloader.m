...
//        self.HTTPHeaders[field] = value;
        [self.HTTPHeaders setObject:value forKey:field];    // modify by zy  兼容4.0及以下编译器
    }
    else
    {
...
//    return self.HTTPHeaders[field];
    return [self.HTTPHeaders objectForKey:field];   // modify by zy  兼容4.0及以下编译器
}
...
//                SDWebImageDownloaderProgressBlock callback = callbacks[kProgressCallbackKey];
                SDWebImageDownloaderProgressBlock callback = [callbacks objectForKey:kProgressCallbackKey]; // modify by zy  兼容4.0及以下编译器
...
//                SDWebImageDownloaderCompletedBlock callback = callbacks[kCompletedCallbackKey];
                SDWebImageDownloaderCompletedBlock callback = [callbacks objectForKey:kCompletedCallbackKey];   // modify by zy  兼容4.0及以下编译器
...
//        NSMutableArray *callbacksForURL = self.URLCallbacks[url];
        NSMutableArray *callbacksForURL = [self.URLCallbacks objectForKey:url];     // modify by zy  兼容4.0及以下编译器
        NSMutableDictionary *callbacks = NSMutableDictionary.new;
//        if (progressBlock) callbacks[kProgressCallbackKey] = [progressBlock copy];
//        if (completedBlock) callbacks[kCompletedCallbackKey] = [completedBlock copy];
        if (progressBlock) [callbacks setObject:[progressBlock copy] forKey:kProgressCallbackKey];      // modify by zy  兼容4.0及以下编译器
        if (completedBlock) [callbacks setObject:[completedBlock copy] forKey:kCompletedCallbackKey];
        [callbacksForURL addObject:callbacks];
//        self.URLCallbacks[url] = callbacksForURL;
        [self.URLCallbacks setObject:callbacksForURL forKey:url];       // modify by zy  兼容4.0及以下编译器
...
//        callbacksForURL = self.URLCallbacks[url];
        callbacksForURL = [self.URLCallbacks objectForKey:url];     // modify by zy  兼容4.0及以下编译器
    });
