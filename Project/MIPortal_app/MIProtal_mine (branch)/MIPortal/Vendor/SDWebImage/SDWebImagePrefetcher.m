...
//    [self.manager downloadWithURL:self.prefetchURLs[index] options:self.options progress:nil completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished)
    [self.manager downloadWithURL:[self.prefetchURLs objectAtIndex:index] options:self.options progress:nil completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished)  // modify by zy  兼容4.0及以下编译器
    {
        if (!finished) return;
        self.finishedCount++;
