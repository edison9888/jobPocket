    ...
    if (self.showType == 0) {  //卡密充值按钮
            ...
            ([dictionary[@"OrderStatusCode"] intValue] == 11109))
        {
            ...
            if (![Global sharedInstance].isLogin ) {
                ...
                rect.origin.x=7;
                rect.size.width =self.scrollView.frame.size.width-14;
                cancelButton.frame=rect;
                chargeView1.frame =rect;
            }
...
- (void)onCancelAction
{
    ...
    [MyAppDelegate.cserviceEngine postXMLWithCode:@"cancelOrder"
                                      ...
                                      onSucceeded:^(NSDictionary *dict) {
                                          ...
                                          NSString *engineErrorInfo=engineError.localizedDescription;
                                          if (engineErrorInfo.length>0&&![engineErrorInfo isEqual:[NSNull null]]&&![@"null" isEqualToString:engineErrorInfo]) {
                                              [SVProgressHUD showErrorWithStatus:engineErrorInfo];
                                          }
                                          if (engineError.userInfo[@"ResultCode"])
                                          {
                                              if ([engineError.userInfo[@"ResultCode"] isEqualToString:@"X104"])
