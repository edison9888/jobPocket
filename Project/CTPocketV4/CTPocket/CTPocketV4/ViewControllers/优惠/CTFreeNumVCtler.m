...
@implementation CTFreeNumVCtler
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        //        self.areaInfo =  [CTHelperMap shareHelperMap].areaInfo; //不考虑查询失败
...
- (void)viewDidLoad
{
    ...
    UIView *tipView = [[UIView alloc] initWithFrame:
                       CGRectMake(10, 0,
                                  CGRectGetWidth(self.view.frame)-5,
    ...
    prettyNumInfoView.backgroundColor = [UIColor clearColor];

    //监听搜索消息
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(searchNumNotification:)
                                                 name:SEARCHFREE_MSG object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
    ...
    self.qryNumByCnlOpt =[MyAppDelegate.cserviceEngine
                          ...
                          onSucceeded:^(NSDictionary *dict) {
                              ...
                              if (wself.type == normalModel) {
                                  ...
                                  [wself matchCollectionData:array];
                                  wself.isLoading = NO ;
                                  
                                  if (wself.dataArray.count == 0) {
                                      wself.tipView.hidden = NO;
                                  } else {
                                      wself.tipView.hidden = YES ;
                                  }
                              }
                              else {
                                  if (array.count<20) {
                                      wself.isSearchFinish = YES ;
                                  }else{
                                      wself.searchPageIndex ++;
                                      wself.isSearchFinish = NO ;
                                  }
                                  
                                  [wself matchCollectionData:array];
                                  wself.isSearchLoading = NO ;
                                  
                                  if (wself.searchArray.count == 0) {
                                      wself.tipView.hidden = NO ;
                                  } else {
                                      wself.tipView.hidden = YES ;
                                  }
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section==1) {
        ...
        if (self.type == normalModel) {
            ...
            if(cell == nil)
            {
                cell = [[CTPrettyNumViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier ViewListCellType:freeCellType];
                cell.selectionStyle = UITableViewCellSelectionStyleNone;
            }
