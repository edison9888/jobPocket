...
- (void)viewDidLoad
{
    ...
    
    {
        ...
        float x=22,yOriginal = 0;
        {
            ...
            txtMoney.frame = CGRectMake(23,yOriginal, 270, 35);
...
- (void) qryDiscount
{
    ...
    NSString *Amount = @"50";//[self.cashdict objectForKey:[NSString stringWithFormat:@"%d",index]];
    NSString *Type = @"101" ;
...
- (void) showDiscountImage : (BOOL) isShow discountRate : (NSString *)DiscountRate
{
    for (int i=0; i<6; i++) {
        ...
        if (isShow) {
            ...
            lab.text = @"9";//[NSString stringWithFormat:@"%0.1f",[DiscountRate floatValue]*10] ;
        }else{
            imageView.hidden = YES ;
        }
    }
...
- (void)keyboardWillShow:(NSNotification *)notification
{
    if(self.clickView == self.carPassWrdTextField)
        ...
        [self.scrollView setContentOffset:CGPointMake(0, 135) animated:YES];
}
