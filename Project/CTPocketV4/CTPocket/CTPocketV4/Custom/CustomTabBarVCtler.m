...
//added by huangfq 2014-6-6
#define kGuideViewCount  5 //引导页的数量
        ...
        (![[settings objectForKey:@"Version"] isEqualToString:oldV]))
    {
        ...
        self.guideView.backgroundColor = [UIColor clearColor];
        {
           ...
           self. guideScrollView.backgroundColor = [UIColor clearColor];
            self.guideScrollView.contentSize = CGSizeMake(self.guideView.bounds.size.width*customTabGuideViewPage, self.guideView.bounds.size.height);
           ...
           self. guideScrollView.showsHorizontalScrollIndicator = NO;
            if (iPhone5)
            {
                ...
                //modified by huangfq 2014-6-6
                for (int i = 0; i < kGuideViewCount; i++) {
...
#endif
                //modified by huangfq 2014-6-6
                CGFloat xOffset = (kGuideViewCount - 1) * 320;
                UIButton *btn = [UIButton buttonWithType:UIButtonTypeCustom];
                [btn setFrame:CGRectMake(xOffset+100, 348+originY, 120, 60)];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    if (page >= kGuideViewCount) //kGuideViewCount 引导页的张数
    {
        [self removeView];
        return;
    }
