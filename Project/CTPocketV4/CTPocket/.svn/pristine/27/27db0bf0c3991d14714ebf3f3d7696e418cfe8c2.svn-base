...
@interface CTContractPayDoneVCterl ()
...
@implementation CTContractPayDoneVCterl
...
- (void) setPhoneInfo
{
    if (self.info&& self.info[@"item"]) {
        if (self.info[@"item"]/*[@"PhoneNumber"]*/) { //modified by shallow 2014-4-23
            NSString *str;
            ...
            {
                str = [NSString stringWithFormat:@"%@",self.salesProInfo[@"SalesProName"]];
            }
...
- (void) onLeftBtnAction:(id)sender
{
    ...
    for (id vc in VCtlerArary) {
        ...
        if ([vc isKindOfClass:[CTPreferentialVCtler class]]) {
            
            for (id vc in VCtlerArary) {
                
                if ([vc isKindOfClass:[CTPhoneListViewController class]]) {
                    isFound = YES ;
        ...
        else if ([vc isKindOfClass:[CTHomeVCtler class]]) {
