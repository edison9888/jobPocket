...
{
    if (!_mPageControl) {
        _mPageControl = [[StyledPageControl alloc] initWithFrame:CGRectMake(120, 112+20, 160, 20)];
...
//    [rBtn setImage:[UIImage imageNamed:@"LeXiang3G_RArrow@2x"] forState:UIControlStateNormal];
   ...
   forControlEvents:UIControlEventTouchUpInside];
    ...
    // 赠品
    self.giftLabel = [[UILabel alloc] initWithFrame:CGRectMake(50, 142+10, 220, 16)];
    ...
    // 话费返还
    self.hfBackLabel = [[UILabel alloc] initWithFrame:CGRectMake(20, 152+16+4, 260, 32)];
...
//        if ([defaults objectForKey:@"YunKa"])
//        {
//            int inx = [[defaults objectForKey:@"YunKa"] integerValue];
...
//        }
        ...
        if ([self.packages count]>inx)
        {
            packageItem = self.packages[inx];
        } else {
            if (self.packages[0])
            {
                packageItem = self.packages[0];
