...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (_homeIndex == 1)
    {
        ...
        if ([Global sharedInstance].isShowBusiProc)
        {
            if (indexPath.row != [[Global sharedInstance].configArray count])
            {
                ...
                __weak typeof(navigation) weakNavigation = navigation;
    ...
    if (_homeIndex == 4)
    {
        ...
        switch (indexPath.row) {
            ...
            case 2:
            {
                CTBroadbandVCtler * broadband = [[CTBroadbandVCtler alloc] initWithNibName:@"CTBroadbandVCtler" bundle:nil];
                [navigation pushViewController:broadband animated:YES];
            }break;
            default:
                break;
        }
