...
static ES_PBField *sDefaultInstance = nil;

+ (void)initialize {
	if (self == [ES_PBField class]) {
		sDefaultInstance = [[ES_PBField alloc] init];
	}
}
...
- (int32_t)getSerializedSize:(int32_t)fieldNumber {
	...
	if (varintValues) {
		...
		for (NSUInteger i = 0; i < count; ++i) {
			result += ES_computeInt64Size(fieldNumber, varintValues[i]);
		}
	}
	...
	if (fixed32Values) {
		...
		for (NSUInteger i = 0; i < count; ++i) {
			result += ES_computeFixed32Size(fieldNumber, fixed32Values[i]);
		}
	}
	...
	if (fixed64Values) {
		...
		for (NSUInteger i = 0; i < count; ++i) {
			result += ES_computeFixed64Size(fieldNumber, fixed64Values[i]);
		}
	}

	for (NSData *value in _lengthDelimitedArray) {
		result += ES_computeDataSize(fieldNumber, value);
	}

	for (ES_PBUnknownFieldSet *value in _groupArray) {
		result += ES_computeUnknownGroupSize(fieldNumber, value);
	}
...
- (int32_t)getSerializedSizeAsMessageSetExtension:(int32_t)fieldNumber {
	...
	for (NSData *value in _lengthDelimitedArray) {
		result += ES_computeRawMessageSetExtensionSize(fieldNumber, value);
	}
...
- (void)writeDescriptionFor:(int32_t) fieldNumber
  ...
  for (ES_PBUnknownFieldSet* value in self.groupArray) {
    [output appendFormat:@"%@%d: [\n", indent, fieldNumber];
