...
@property (nonatomic,strong) CustomTabBarVCtler * tabBarController ;
...
- (instancetype)init:(UIViewController *)tabbar type:(JUMPTYPE *)pushType
{
    self = [super init];
    if (self) {
        _tabBarController = (CustomTabBarVCtler *)tabbar;
                ...
                if ([clsname isEqualToString:viewControllers[i]])
                {
                    if ([clsname isEqualToString:@"CTRechargeVCtler"])
                    {
                        ...
                        if (_pushType == push) {
                            ...
                            [self.tabBarController.viewControllers[i] popViewControllerAnimated:YES];
                            return;
                        }
