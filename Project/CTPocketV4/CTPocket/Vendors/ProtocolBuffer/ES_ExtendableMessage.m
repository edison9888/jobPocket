...
@implementation ES_PBExtendableMessage
...
- (BOOL) isInitialized:(id) object {
  ...
  } else if ([object conformsToProtocol:@protocol(ES_PBMessage)]) {
...
- (void) writeExtensionsToCodedOutputStream:(ES_PBCodedOutputStream*) output
  ...
  for (NSNumber* number in sortedKeys) {
    ...
    if (fieldNumber >= startInclusive && fieldNumber < endExclusive) {
      id<ES_PBExtensionField> extension = [extensionRegistry objectForKey:number];
