...
- (void)viewDidLoad
{
    ...
    scrollView.backgroundColor =  PAGEVIEW_BG_COLOR;//[UIColor colorWithRed:230/255. green:230/255. blue:230/255. alpha:1]; // modified by zy
...
//        [self alert:@"请输入正确的手机号码" leftbutton : @"确定" rightbutton : nil];
        return;
    }
...
//            [self alert:@"充值密码应为18位" leftbutton : @"确定" rightbutton : nil];
        ...
        __block CTFlowRechargeVCtler *mySelf = self;
        [[FlowCardValidata shareFlowCardValidata]flowValidatePhoneNumber:phone finish:^(NSDictionary *resultDictionary)
        {
            if (resultDictionary == nil || ![resultDictionary respondsToSelector:@selector(objectForKey:)]) {
                [self alert:@"流量充值目标号码查询失败，请您稍后再试" leftbutton:@"确定" rightbutton:@"取消"];
            } else {
                ...
                if (Data && [Data respondsToSelector:@selector(objectForKey:)]) {
                    ...
                    if (code && [code respondsToSelector:@selector(intValue)] && [code intValue] == 0) {
                        ...
                        if (reason && [reason isKindOfClass:[NSString class]]) {
                            [mySelf alert:reason leftbutton:nil rightbutton:nil];
                        } else {
                            [mySelf alert:@"流量充值目标号码查询失败，请您稍后再试" leftbutton : nil rightbutton : nil ];
                        }
                    }
                }
            }
...
- (void) FCInfo
{
    ...
    //type = 2 充流量   1 充花费
    NSDictionary *params = [NSDictionary dictionaryWithObjectsAndKeys:phoneNumber, @"phoneNbr", password, @"cardPwd", ESHORE_ShopId, @"ShopId", @"2", @"Type", nil];
...
- (void)bankCardInfo
{
    ...
    NSDictionary *params = [NSDictionary dictionaryWithObjectsAndKeys:phoneNumber, @"PhoneNumber", money, @"PayAmount", UserId, @"UserId", ESHORE_ShopId, @"ShopId", @"2", @"Type", nil];
...
- (void) qryDiscount
{
    ...
    NSString *Amount = @"20";//[self.cashdict objectForKey:[NSString stringWithFormat:@"%d",index]];
    NSString *Type = @"2" ;
...
- (void)keyboardWillShow:(NSNotification *)notification
{
    [UIView animateWithDuration:0.25 animations:^{
        if (self.clickView == self.carPassWrdTextField) {
            _scrollView.frame = CGRectMake(0, -40, self.view.frame.size.width, self.view.frame.size.height);
