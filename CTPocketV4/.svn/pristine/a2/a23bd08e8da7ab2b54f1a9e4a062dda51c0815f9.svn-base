...
+ (CGFloat)CellHeightWithConfigList:(NSArray *)configList
{
    ...
    NSUInteger floors = itemCount/countOneFloor;
    if (floors == 0) {
        floors = 1;
    } else {
        if (itemCount%floors != 0) {
            floors += 1;
        }
    }
...
- (void)setConfigList:(NSArray *)configList
{
    ...
    /*
    CGRect srcRect=self.textLabel.frame;
    srcRect.origin.x=10;
    UILabel *label=[[UILabel alloc] initWithFrame:srcRect];
    [label setFont:[UIFont systemFontOfSize:12]];
    label.text=model.SpecName;
    [self.contentView addSubview:label];
    self.textLabel.text=@"";
 
    self.textLabel.frame=srcRect;
     **/
