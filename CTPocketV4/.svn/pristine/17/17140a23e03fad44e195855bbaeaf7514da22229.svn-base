//
//  COrderWildProductFillVctrler.m
//  CTPocketV4
//
...
@interface COrderWildProductFillVctrler ()
{
...
@implementation COrderWildProductFillVctrler
...
- (void)viewDidLoad
{
    ...
    
    {
        ...
        UIView *view = [[UIView alloc] initWithFrame:CGRectMake(0, y, CGRectGetWidth(rootSbview.frame), 56)];
        view.backgroundColor = [UIColor colorWithRed:235/255.0
                                               green:235/255.0
                                                blue:235/255.0
                                               alpha:1.0];
        UIButton* btnRead = [UIButton buttonWithType:UIButtonTypeSystem];
        CGRect trect= CGRectMake(ceilf((CGRectGetWidth(view.frame) - 260)/2.0), ceilf((CGRectGetHeight(view.frame) - 36)/2.0), 260, 36);
...
-(void)LayoutScollview{
    UIScrollView* rootSbview = (UIScrollView*)[self.view viewWithTag:kTagRootScrollview];
    {
        ...
        BOOL bare = [[NSUserDefaults standardUserDefaults]boolForKey:@"BareMetal"];
        if (bare) {
            
            NSLog(@"是裸机，不显示入网信息，呵呵，");
        }
        else
        {
            {
            ...
            trect   = CGRectMake(xoffset, yoffset+20,255, 35);
        ...
        UIImageView *dividing = [[UIImageView alloc] init];
        ...
        {
            yoffset = CGRectGetMaxY(trect) +20;
            dividing.frame = CGRectMake(0, yoffset, CGRectGetWidth(rootSbview.frame), 1);
        ...
        image = [UIImage imageNamed:@"prettyNum_dividingLine_ver.png"];
        dividing = [[UIImageView alloc] initWithFrame:CGRectMake(0, yoffset, CGRectGetWidth(rootSbview.frame), 1)];
        dividing.image = image;
        [rootSbview addSubview:dividing];
        ...
        yoffset += 10;
        trect = CGRectMake(xoffset-10, yoffset,CGRectGetWidth(rootSbview.frame)-xoffset, 260);
        CInvoiceEditView *invoiceView = [[CInvoiceEditView alloc]initWithFrame:trect];
        invoiceView.delegate = self;
        invoiceView.tag = kTagInvoiceView;
        [rootSbview addSubview:invoiceView];
        
        // 下一步
        yoffset += CGRectGetHeight(trect) + 35;
        trect = CGRectMake(xoffset, yoffset,CGRectGetWidth(rootSbview.frame)-2*xoffset,35);
        {
            UIButton* btnRead = [UIButton buttonWithType:UIButtonTypeSystem];
            btnRead.frame = trect;
        ...
        UIButton *bgBtn = (UIButton *)[rootSbview viewWithTag:kBgBtn];
        
        yoffset += CGRectGetHeight(rootSbview.frame) + 10;
        bgBtn.frame = CGRectMake(0, 0, rootSbview.contentSize.width, yoffset);
...
-(void)onKeyboardHidden:(id)sender
{
    ...
    CInvoiceEditView *invoiceView = (CInvoiceEditView*)[self.view viewWithTag:kTagInvoiceView];
    if (invoiceView) {
        [invoiceView hiddenKeyBord];
    }
...
//        vc.info = self.info;
//        [self.navigationController pushViewController:vc animated:YES];
        CTContractOrderConfirmVCtler *vc = [CTContractOrderConfirmVCtler new];
        
        CInvoiceEditView *invoiceView = (CInvoiceEditView*)[self.view viewWithTag:kTagInvoiceView];
        NSMutableDictionary *invoiceDic = [NSMutableDictionary new];
        invoiceDic = [NSMutableDictionary dictionaryWithDictionary:_defaultRecipDict];
        
        if (invoiceView.checkNum == 1) {
            if (0 == invoiceView.companyField.text.length) {
                SIAlertView *alertView =
                [[SIAlertView alloc] initWithTitle:nil
                                        andMessage:@"发票抬头还没有告诉我们哦~"];
                [alertView addButtonWithTitle:@"确定"
                                         type:SIAlertViewButtonTypeDefault
                                      handler:^(SIAlertView *alertView){
                                      }];
            ...
            {
                [invoiceDic setObject:invoiceView.companyField.text forKey:@"InvoiceInfoType"]; // 发票抬头
            }
            if (invoiceView.metrialNum == 1) {
                [invoiceDic setObject:@"通信器材" forKey:@"InvoiceContentType"]; // 发票内容
                
                // 发票接口
                {
                    vc.InvoiceType = @"0";
                    vc.InvoiceContentType = @"0";
                    vc.InvoiceInfo =invoiceView.companyField.text;
                    vc.InvoiceInfoType = @"1";
                }
            }
            else
            {
                [invoiceDic setObject:@"明细" forKey:@"InvoiceContentType"]; // 发票内容
                
                // 发票接口
                {
                    vc.InvoiceType = @"0";
                    vc.InvoiceContentType = @"1";
                    vc.InvoiceInfo =invoiceView.companyField.text;
                    vc.InvoiceInfoType = @"1";
                }
            }
        ...
        {
            [invoiceDic setObject:@"个人" forKey:@"InvoiceInfoType"]; // 发票抬头
            if (invoiceView.metrialNum == 1) {
                [invoiceDic setObject:@"通信器材" forKey:@"InvoiceContentType"]; // 发票内容
        ...
        //订单确认一系列参数传递  by shallow
        
        vc.SalesName =  self.dataModel.SalesProdName;            //订单商品名
        vc.Phone = self.phoneData.PhoneNumber;                   //入网号码
        vc.MinAmount = [NSString stringWithFormat:@"最低消费%@", self.dataModel.Package.MinAmount];  //最低消费
        
        //生成订单
        vc.SalesProdId = self.SalesProdId;          //销售品ID
        vc.PhoneNumber = self.phoneData.PhoneNumber;             //手机号码
        vc.AbType = self.dataModel.AbType;                       //销售品AB类型
        vc.SalesProdType = self.dataModel.SalesProdType;         //销售品类型
        
//        CTContractOrderConfirmVCtler *vc = [[CTContractOrderConfirmVCtler alloc]init];
        vc.selRecipientDict = invoiceDic;
        vc.uName = tfUname.text;
        vc.uID = tfUcode.text;
        vc.uAddress = tfUAddress.text;
        vc.uPostCode = @"-";
    ...
    }else //用户未登录
    {
        ...
        [selDict setObject:creView.phone.text
                    forKey:@"CusMobile"];
        [selDict setObject:tfUname.text?tfUname.text:@""
                    forKey:@"CusName"];
        ...
        CInvoiceEditView *invoiceView = (CInvoiceEditView*)[self.view viewWithTag:kTagInvoiceView];
        NSMutableDictionary *invoiceDic = [NSMutableDictionary new];
        invoiceDic = [NSMutableDictionary dictionaryWithDictionary:selDict];
        // modefied by shallow 2014-04-04
        if (invoiceView.checkNum == 1) {
            if (0 == invoiceView.companyField.text.length) {
                SIAlertView *alertView =
                [[SIAlertView alloc] initWithTitle:nil
...
-(void)onHiddenInvoiceView:(id)sender
{
    [self onKeyboardHidden:nil];
}
