...
@interface CTLovingPrettyNumVCtler ()
...
@implementation CTLovingPrettyNumVCtler
...
//    [self.view addSubview:downImageView];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(searchNumNotification:)
                                                 name:SEARCHLOVING_MSG object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
...
#if 0   // modified by zy, 2014-02-21
    ...
    [view setCollectedBlock: ^(int collectIndex,BOOL collectedStatus)
     {
         ...
         if (strongSelf.type == normalModel) {
             if (collectedStatus) {
                 CTPrettyNumData *prettyData = (CTPrettyNumData *)strongSelf.dataArray[collectIndex];
                 ...
                 [[NSNotificationCenter defaultCenter] postNotificationName:@"UPDATADATACOLLECTED" object:strongSelf];
             }else{
                 CTPrettyNumData *prettyData =  (CTPrettyNumData *)strongSelf.dataArray[collectIndex];
             ...
             if (collectedStatus) {
                 CTPrettyNumData *prettyData = (CTPrettyNumData *)strongSelf.searchArray[collectIndex];
                 [prettyData setValue:@"1" forKey:@"isCollected"];
                 //取消收藏
                 NSMutableDictionary *dict = [NSMutableDictionary dictionaryWithDictionary:prettyData.dictionaryRepresentation];
                 [[CTQryCollected shareQryCollected].collectedMutableDict addObject:dict];
...
- (void) selectedCityNotification : (NSNotification *)notification
{
    if (self.qryNumByCnlOpt) {
        ...
        self.qryNumByCnlOpt = nil;          // modified by zy, 2014-02-21
    }
...
- (void)onQryNumByCnlSuccess:(NSDictionary *)dict
{
    ...
    } else if(Items && [Items isKindOfClass:[NSDictionary class]]){
        array = [NSArray arrayWithObjects:Items, nil];
    }
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section==1) {
        ...
        if (self.type == normalModel) {
            static NSString *CellIdentifier = @"LovingCell";
            CTPrettyNumViewCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
            if(cell == nil)
            {
                cell = [[CTPrettyNumViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier ViewListCellType:lovingCellType];
                cell.selectionStyle = UITableViewCellSelectionStyleNone;
            }
