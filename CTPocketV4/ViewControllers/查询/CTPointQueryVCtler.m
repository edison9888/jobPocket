...
- (void)viewDidLoad
{
    ...
    //已用积分下面的提示语
    {
        ...
        [button setImage:[UIImage imageNamed:@""] forState:UIControlStateNormal];
        [button setTitleColor:[UIColor darkGrayColor] forState:UIControlStateNormal];
        [button setTitle:@"本月已用积分" forState:UIControlStateNormal];
        [button.titleLabel setFont:[UIFont systemFontOfSize:13]];
        button.enabled = NO;
        [self.view addSubview:button];
    ...
    xOffset = 20;
    float verticalDistance = 12, visibleWidth = CGRectGetWidth(self.view.bounds) - 2 * xOffset;
...
//                [self pointCosumeHistory:DeviceNo];
                [[IgInfo shareIgInfo] igInfoWithDeviceNo:DeviceNo CustId:CustId finishBlock:^(NSDictionary *resultParams, NSError *error) {
                    ...
                    [self igProdListWithMaxPrice:[Integral integerValue]];
                }];
            } else {
                self.igOrderListLoading = NO;
                self.igProdListLoading = NO;
...
- (void)igProdListWithMaxPrice:(NSUInteger)MaxPrice
{
    ...
    __weak typeof(self) weakSelf = self;
    [self.igProdListNetworking igProdListWith:1 PageSize:20 Sort:2 MinPrice:0 MaxPrice:MaxPrice CommdityId:@"08" KeyWord:nil CategoryId:nil SmallCategoryId:nil finishBlock:^(NSDictionary *resultParams, NSError *error) {
        __strong typeof(weakSelf) strongSelf = weakSelf;
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    if (1) {
        ...
        [threeSubView.centerButton setTitle:[NSString stringWithFormat:@"  我可兑换%@", soMuchWhiteSpace] forState:UIControlStateNormal];
