...
//            str = @"王海  010-58507321\n北京市海淀区杏石口路99号3棟西山赢俯D座100093";
    ...
    //发票信息
    {
        ...
        [self.scrollView addSubview:imageView];
        {
            ...
            if (!InvoiceInfo) {
                ...
                NSString *name = InvoiceInfoType > 0? @"个人":@"单位";
                int InvoiceContentType = [[InvoiceInfo objectForKey:@"InvoiceContentType"]intValue];
                NSString *content = InvoiceContentType > 0?@"通信器材":@"商品明细";
                label2.text = name ;
                label3.text = content;
            }
        }
    ...
    if (self.showType == 0) {  //卡密充值按钮
            ...
            ([dictionary[@"OrderStatusCode"] intValue] == 11109))
        {
            ...
            [button addTarget:self action:@selector(onPayAction) forControlEvents:UIControlEventTouchUpInside];
            [self.scrollView addSubview:button];
