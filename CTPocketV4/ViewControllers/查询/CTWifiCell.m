...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        // Initialization code
        self.frame = CGRectMake(0, 0, kCTWifiTableCellWidth, kCTWifiTableCellHight);
...
- (void)layoutSubviews
{
    ...
    if ([self.wifiInfo objectForKey:@"hp_name"])
    {
        [name appendString:[self.wifiInfo objectForKey:@"hp_name"]];
    }
    _nameLab.text = name;
    
    _addrLab.text = @"";
    if ([self.wifiInfo objectForKey:@"hp_address"])
    {
        _addrLab.text    = [NSString stringWithFormat:@"地址: %@", [self.wifiInfo objectForKey:@"hp_address"]];
    }
    
    id distance = [self.wifiInfo objectForKey:@"distance"];
    ...
    if (distance && [distance respondsToSelector:@selector(floatValue)])
    {
        ...
        {
            [_distanceBtn setTitle:[NSString stringWithFormat:@"%@米", [self.wifiInfo objectForKey:@"distance"]] forState:UIControlStateNormal];
        }
        else
        {
            [_distanceBtn setTitle:[NSString stringWithFormat:@"%.2f公里", [[self.wifiInfo objectForKey:@"distance"] floatValue]/1000] forState:UIControlStateNormal];
        }
    }
}
