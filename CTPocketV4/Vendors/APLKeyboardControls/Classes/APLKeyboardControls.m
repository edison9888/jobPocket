...
- (UIBarButtonItem *)doneButton {
    if (!_doneButton) {
         _doneButton = [[UIBarButtonItem alloc] initWithTitle:@"完成" style:UIBarButtonItemStyleBordered target:self action:@selector(closeInput:)];
...
- (UIResponder*)currentInput {
    if (![_currentInput isFirstResponder]) {
        ...
        if (index>=self.inputFields.count) {
            return nil;
        }
